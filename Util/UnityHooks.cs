...
namespace SteamVR_Standalone_IL2CPP.Util
{
   ...
   : Il2CppSystem.Delegate.Combine(Camera.onPreCull, (Camera.CameraCallback)new Action<Camera>(OnPreCull)).Cast<Camera.CameraCallback>()
        ...
        private static void OnPreRender(Camera cam) { if (OnPreRenderCam == null || !OnPreRenderCam.enabled) OnPreRenderCam = cam; if (OnPreRenderCam == cam) InvokeOnBeforeRender(); }

        private static void OnPreCull(Camera cam) { if (OnPreCullCam == null || !OnPreCullCam.enabled) OnPreCullCam = cam; if (OnPreCullCam == cam) InvokeOnBeforeCull(cam); }
